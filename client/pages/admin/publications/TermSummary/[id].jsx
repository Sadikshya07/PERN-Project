...
export default function UpdateAnalysisReport() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await TermSummaryFinder.get(`/${id}`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
      const response = await TermSummaryFinder.put(
        `/${id}`, {
        name,
      ...
      router.push(`/admin/publications/termsummary`);
  };

  return (
    <div>
      <Head>
        <title>Update-Term Summary</title>
